...
def LFT_unit_test(window, res_y=128, res_x=128, bS=1, avDev=torch.device("cpu"), pS=1, eps=1e-12, H=2):
    ...
    S_fft,_ = LFT(S + eps, window,None, x_stride=H, y_stride=H, padding=pS,useCOM=False)
    # T = getPhaseDiff(S_fft, S_fft)
    lLFT,_=LFT(S + eps, window,None, x_stride=H, y_stride=H, padding=pS,useCOM=False)
    bLFT,_=LFT(S + eps, window,None, x_stride=H, y_stride=H, padding=pS,useCOM=False)
