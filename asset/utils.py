...
def clmp(a, soft=False,skew=10):
    if soft:
        return torch.sigmoid(-(skew/2.) + (a * skew)) #GOTO: https://www.desmos.com/calculator and type: \frac{1}{1+e^{-\left(-\left(\frac{s}{2}\right)+\left(sx\right)\right)}}
    return a.clamp(0, 1)
...
def createPhaseDiff(avgRecX, avgRecY, shape):
    global recIndex
    # avgRecX = torch.Size([BS*CS*L_y*L_x]) ! avgRecX = torch.Size([BS*CS*L_y*L_x])
    # print(avgRecX.shape, avgRecY.shape, shape)
    BS, H, W, _ = shape
    ...
    # print(angleX.shape,recIndex.shape,"@")
...
def getAvgDiff(rot, energy=None, step=1, axis=0, dims=(1, 2), untilIndex=None,variance=False):
    ...
    # dmsg('rot.shape')#torch.Size([BS*CS*L_y*L_x, N_p, N_p, 2])
    oAxis = 1 if axis == 0 else 0
...
def show_phase_diff(pd_list, gt, config, title,clear_motion=False):
    ...
    for i in range(config.sequence_length):
        ...
        if i == 0:
            ...
            ax.quiver(xx, yy, visX[0,:,:,0].cpu(), visY[0,:,:,0].cpu(), alpha=0)
            if showStillVisualization:
                axT[i].quiver(xx, yy, visX[0,:,:,0].cpu(), visY[0,:,:,0].cpu(), alpha=0)
        else:
            with torch.no_grad():
            ...
            for chan in range(visY.shape[-1]):
                if not color_like_flow or visY.shape[-1]>1:
                    ...
                    alpha =  float((chsum[chan]/chsum.sum()).item()>0.1)
                    colors = np.array(list(colors)+[alpha])

                ax.quiver(xx, yy, visX[0,:,:,chan].cpu(), visY[0,:,:,chan].cpu(), color=colors, scale=2.5 / config.stride,
                        units='xy')  # scale is important here, might have to adjust to each dataset #, pivot='mid'
                if showStillVisualization:
                    axT[i].quiver(xx, yy, visX[0,:,:,chan].cpu(), visY[0,:,:,chan].cpu(), color=colors, scale=2.5 / config.stride,
                                units='xy')  # scale is important here, might have to adjust to each dataset #, pivot='mid'
...
class DenseNetLikeModel(nn.Module):
    def __init__(self, inputC,outputC, layerCount=4, hiddenF=24, filterS=3,gain=1,nonlin='ReLU',lastNonLin=False,initW=True,bn=True):
        super().__init__()
...
class LocationAwareConv2d(torch.nn.Conv2d):
    def __init__(self,w,h,in_channels, out_channels, kernel_size, stride=1, padding=0, dilation=1, groups=1, bias=True,CCount=4):
        super().__init__(in_channels, out_channels, kernel_size, stride=stride, padding=padding, dilation=dilation, groups=groups, bias=bias)
        self.CCount=CCount
        self.locationBias=torch.nn.Parameter(torch.zeros(w,h,self.CCount))
        self.w=w
        self.h=h
...
class UNet(nn.Module):
    ...
    def __init__(self, in_ch,out_ch,full=True,hd=16,pad=0,locAware=False,w=None,h=None):
        super().__init__()
        self.locAware= locAware and w is not None and h is not None     
        if self.locAware:
            self.loc1 = LocationAwareConv2d(w//2,h//2,hd,hd,3,1,1)
